      ...
      <div className="flex-shrink-0 p-4 sm:p-6 bg-white border-b border-gray-200">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
          ...
          <button
            ...
            className="w-full sm:w-auto bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center space-x-2 flex-shrink-0"
          >
            ...
            >
              <div className="flex items-center space-x-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="text-base sm:text-lg font-semibold text-gray-900">I Can View</h3>
                  <p className="text-xs sm:text-sm text-gray-600">Health data I have access to ({friends.length})</p>
                </div>
              </div>
                  ...
                  <div className="px-4 sm:px-6 pb-4 border-t border-gray-200">
                    {friends.length === 0 ? (
                      <div className="text-center py-6">
                        <Eye className="h-10 w-10 sm:h-12 sm:w-12 text-gray-400 mx-auto mb-3" />
                        <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-2">No reading access yet</h3>
                        <p className="text-sm text-gray-600 mb-4">Request access to view others' health readings</p>
                        <button
                          onClick={() => setShowAddFriend(true)}
                            ...
                            <div className="flex items-center space-x-3 min-w-0 flex-1">
                              ...
                              <div className="min-w-0 flex-1">
                                <button
                                  ...
                                  className="font-medium text-gray-900 hover:text-blue-600 transition-colors text-left text-sm sm:text-base truncate block w-full"
                                >
                                  {permission.owner_name || permission.owner_email?.split('@')[0] || 'User'}
                                </button>
                                <p className="text-xs sm:text-sm text-gray-500 truncate">{permission.owner_email || 'No email available'}</p>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            >
              <div className="flex items-center space-x-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="text-base sm:text-lg font-semibold text-gray-900">Who Can View Mine</h3>
                  <p className="text-xs sm:text-sm text-gray-600">People who can view my health data ({myViewers.length})</p>
                </div>
              </div>
              <div className="flex-shrink-0 ml-2">
              ...
              {openAccordion === 'my-access' && (
                ...
                >
                  <div className="px-4 sm:px-6 pb-4 border-t border-gray-200">
                    {myViewers.length === 0 ? (
                      <div className="text-center py-6">
                        <Shield className="h-10 w-10 sm:h-12 sm:w-12 text-gray-400 mx-auto mb-3" />
                        <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-2">No one has access yet</h3>
                        <p className="text-sm text-gray-600">No users currently have permission to view your health readings</p>
                      </div>
                    ) : (
                      <div className="space-y-3 mt-4 max-h-80 overflow-y-auto">
                          ...
                          <div key={permission.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                            <div className="flex items-center space-x-3 min-w-0 flex-1">
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <p className="text-xs sm:text-sm text-gray-500 truncate">{permission.viewer_email || 'No email available'}</p>
                                <p className="text-xs text-gray-400">
                                  Access granted {new Date(permission.created_at).toLocaleDateString()}
                                </p>
                              </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            >
              <div className="flex items-center space-x-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="text-base sm:text-lg font-semibold text-gray-900">Requests</h3>
                  <p className="text-xs sm:text-sm text-gray-600">Pending access requests ({receivedRequests.length} received, {sentRequests.length} sent)</p>
                </div>
              </div>
              <div className="flex-shrink-0 ml-2">
              ...
              {openAccordion === 'requests' && (
                ...
                >
                  <div className="px-4 sm:px-6 pb-4 border-t border-gray-200">
                    {/* Received Requests */}
                    <div className="mt-4">
                      <h4 className="text-base sm:text-lg font-medium text-gray-900 mb-3">Requests to View My Readings</h4>
                      {receivedRequests.length === 0 ? (
                        <div className="text-center py-4">
                          <Mail className="h-6 w-6 sm:h-8 sm:w-8 text-gray-400 mx-auto mb-2" />
                          <p className="text-sm text-gray-500">No pending reading requests</p>
                        </div>
                      ) : (
                    ...
                    {/* Sent Requests */}
                    <div className="mt-6">
                      <h4 className="text-base sm:text-lg font-medium text-gray-900 mb-3">My Requests for Access</h4>
                      {sentRequests.length === 0 ? (
                        <div className="text-center py-4">
                          <Send className="h-6 w-6 sm:h-8 sm:w-8 text-gray-400 mx-auto mb-2" />
